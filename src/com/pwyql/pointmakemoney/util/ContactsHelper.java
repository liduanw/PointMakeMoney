...
public class ContactsHelper {
	...
	final Handler handler = new Handler() {
		...
		case 1: // 插入中
		    if (callback != null) {
			callback.onRestoring(msg.arg1, msg.obj.toString());
		    }
		    break;
		case 0:
		    if (callback != null) {
			callback.onSuccessed(msg.arg1);
		    }
		    break;
		}
	    }
	...
	new Thread() {
		...
		for (Contact c : contacts) {
		    restoredCount++;
		    handler.obtainMessage(1, restoredCount, 0, c.getDisplayName() + ":" + c.getNumber()).sendToTarget();
		    insertContact(context, c.getDisplayName(), c.getNumber());
		}

		handler.obtainMessage(0, restoredCount, 0).sendToTarget();
	...
	void onSuccessed(int count);

	void onRestoring(int resotredCount, String nameAndNumber);
