 ...
 *
 */
public class WithdrawFormTelfareActivity extends BaseActivity implements OnClickListener {
	...
	setContentView(R.layout.activity_form_telfare);
	...
	postParams.sign = StringUtils.md5("" + 0 + priceType + AppConstants.SIGN_KEY);
	AppServiceImpl.getInstance().withdraw(postParams, new OnWithdrawResponseListener() {
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(WithdrawFormTelfareActivity.this);
	...
	AppServiceImpl.getInstance().loadInfo(postParams, new OnLoadInfoResponseListener() {
		...
		mEtAccountNumber.setText(info.phone);
	    }
